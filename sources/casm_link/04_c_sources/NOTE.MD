...
itera_arrayとitera_array_ptrは同じバイナリ。
配列に、添え字でアクセスする方法とポインタでアクセスする方法は、同じバイナリになる？
...
```
int型の変数が4つあるため、
4 * 4 すると、次の配列の要素になる？（structArray[0]　から　structArray[1]のように添え字が一つ進む）
...
```
sub	sp, sp, #4096  
intの4byte * 1024の分だけスタックを確保している。

mov r0, #4096
とsizeofでは、確保したスタックの領域のバイト数を取得している。
...
@ %bb.0:
	...
	bl	print_size
```
r0に1024（関数の引数に1024を渡す）  
bl malloc後に、r0に確保した領域のメモリを指すポインタが入っている。
mallocした後の変数にsizeofすると、そのポインタの型のsizeが分かる。
...
```
	...
	bl	print_size
```
渡された引数である、r0を2bit左シフトした値が、print_sizeの引数r0になっている。
...
string_ptrは、文字列(.L.str)がある場所へのアドレスを示しているため、
文字列の先頭のアドレスをldrする必要（	ldr	r1, [r0]）がある。
...
func2's a3 address_1 = fffef830
func3's a4 address_1 = fffef81c
address: ff7ee000
result is 507
```
...
# 最終課題: stack_walk2.cでmain_msgを出力せよ
...
We are in func1, func1 msg
We are in func2, func2 message.
We are in func3
address: fffef830
address: fffef840
address: fffef850
